...
**Policy activa**: `amx-revenue-accounting`
**Modo detectado**: `LEGACY_MODERNIZATION` (router score: 19)
**Origen**: Code real de ATOS-NOSHOW-ROBOT .NET 4.7.2 · Alonso · Revenue Accounting AMX

---

## Propósito

Esta carpeta contiene los 3 artefactos generados por Kiro IDE usando el pack AIOS + constraints AMX retro 20-abr, **como evidencia de validación end-to-end** del flujo spec-driven.
...
| Capacidad | Evidencia |
|---|---|
| 10 steering rules cargados | Kiro agent listó los 10 en `config.kiro` incluyendo `amx-constraints-retro-20abr.md` |
| Router detecta LEGACY_MODERNIZATION | Score MIGRATION=19 → mode correcto (aunque UI expone sólo Feature/Bugfix · BUG-001) |
| AMX constraints aplicados sin hand-holding | requirements.md + design.md mencionan EKS · AMX_Relay · CMK exclusiva · amx-r-* · sin ECS/SES/SNS |
| ADR opt A vs B (gate 3 AMX) | 5 ADRs en design.md: Runtime · Secrets · SFTP · SMTP · Orquestación batch |
| Trazabilidad Req X.Y · Design §Z | 109 tasks con referencias cruzadas completas |
| 9 gates AMX retro 20-abr como tasks BLOCKER | Phase 8 · 10 items marcados 🚫 (WIZ+Veracode+Prisma+Tenable separados) |
...
|---|---|---|
| `requirements.md` | 317 | 15 requisitos EARS · funcionales · seguridad · NF · migración dual-run · AMX constraints |
...
| `tasks.md` | 191 | 109 tasks · 8 fases · 10 BLOCKERS (gates AMX) |

---

## Uso previsto

1. **Correo a Alberto Ibrahim Pedraza Cáscar** (creador AM-KIRO) · evidencia de que el pack produce outputs AMX-compliant end-to-end
...
- **No ejecutar las tareas de tasks.md desde esta carpeta** · la implementación de NoShow es scope de Alonso · arranca tras aprobaciones (OYN-AMX access · push rama · workshop SRG scope)
