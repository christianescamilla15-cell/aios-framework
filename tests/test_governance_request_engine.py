...
def test_for_bd_access_context_includes_team_and_resources(builder):
    ctx = builder.for_bd_access(app="sicofav", requester_email="chernandeze@aeromexico.com")
    assert ctx["app"] == "sicofav"
...
def test_for_aws_account_t2_marks_shared(builder):
    ctx = builder.for_aws_account(app="srg", requester_email="chernandeze@aeromexico.com")
    assert "compartida" in ctx["account_type"]


def test_for_aws_account_t0_marks_dedicated(builder):
    ctx = builder.for_aws_account(app="sicofav", requester_email="chernandeze@aeromexico.com")
    assert "dedicada" in ctx["account_type"]


def test_for_cmk_returns_5_aliases_per_env(builder):
    ctx = builder.for_cmk(app="sicofav", requester_email="chernandeze@aeromexico.com")
...
def test_generate_bd_access_writes_md(tmp_path, engine):
    artifacts = engine.generate(
        ...
        requester_email="chernandeze@aeromexico.com",
    ...
    assert artifacts.request_id.startswith("GOV-SICOFAV-BD-")
