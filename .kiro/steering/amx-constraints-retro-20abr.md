...
Reglas obligatorias confirmadas por los arquitectos AMX en la retro del 20-abr-2026 (Antonio Hernández Oropeza + Pedro Emmanuel Abaonza · plus Israel Miguel González Sandoval · Alexis Martínez · Rigoberto Texmayer Gaona · Miguel Rachid).
...
| **SES** | 🔴 PROHIBIDO | **Relay interno AMX** · para correos transaccionales y alertas |
| **SNS (para correos)** | 🔴 PROHIBIDO | **Relay interno AMX** · SNS solo permitido para eventos no-correo |
...
5. **Cuenta AWS AMX** · 24h SLA · correo líder previo + captura al ticket
6. **Diego Zarate** · VPC + subnets (Gateway/pública/privada/datos)
7. **Tickets KMS** · una por servicio · vía GateOne o AMX Chat Service Desk
8. **Escaneos CYBER** · secuencia WIZ → Veracode → Prisma → Tenable
9. **Miguel Rachid** · gate final · Gerente Ciberseguridad AMX
...
| **T1** | EKS o Serverless · OBLIGATORIO | Apps satelitales críticas (SICOFAV · ARC · BSP · etc.) |
...
Cuando un aplicativo tenga el policy `amx-revenue-accounting` activo · AIOS aplica automáticamente estos detectores:

| Situación | Detector AIOS |
|---|---|
| Config con ECS reference | Advierte · señalar alternativa EKS/Serverless |
| Código con `SESSendEmail` / SNS para correos | Advierte · señalar Relay AMX |
| Hardcoded AWS KMS key shared | Advierte · CWE-798 + regla AMX |
| Role sin prefijo `amx-r-*` | Advierte · SCP violation |
| Secretos en config | CWE-522 (Sprint 5.2) + regla AMX |
